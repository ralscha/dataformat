...
public class Table {
	...
	protected int __offset(int vtable_offset) {
		...
		return vtable_offset < this.bb.getShort(vtable)
				? this.bb.getShort(vtable + vtable_offset)
				: 0;
	}
	 ...
	 *
	 * @param offset An `int` index into the Table's ByteBuffer containing the relative
	 * offset.
	 ...
	 *
	 * This allocates a new string and converts to wide chars upon each access, which is
	 * not very efficient. Instead, each FlatBuffer string also comes with an accessor
	 * based on __vector_as_bytebuffer below, which is much more efficient, assuming your
	 * Java program can handle UTF-8 data directly.
	 *
	...
	protected String __string(int offset) {
		...
		int required = (int) (length * decoder.maxCharsPerByte());
	...
	protected int __vector(int offset) {
		offset += this.bb_pos;
		return offset + this.bb.getInt(offset) + SIZEOF_INT; // data starts after the
																// length
	}
	 ...
	 *
	 * This is efficient, since it only allocates a new {@link ByteBuffer} object, but
	 * does not actually copy the data, it still refers to the same bytes as the original
	 * ByteBuffer. Also useful with nested FlatBuffers, etc.
	 *
