...
/**
 * Class that helps you build a FlatBuffer. See the section "Use in Java/C#" in the main
...
public class FlatBufferBuilder {
	...
	static final Charset utf8charset = Charset.forName("UTF-8"); // The UTF-8 character
																	// set used by
																	// FlatBuffers.
	...
	boolean force_defaults = false; // False omits default values from the serialized
									// data.
	...
	/**
	 * Alternative constructor allowing reuse of {@link ByteBuffer}s. The builder can
	 * still grow the buffer as necessary. User classes should make sure to call
	 ...
	 * `ByteBuffer`. This method resets the builder's internal state, but keeps objects
	 * that have been allocated for temporary storage.
	 *
	 * @param existing_bb The byte buffer to reuse.
	 * @return Returns `this`.
	 */
	 ...
	 * e.g. if you write a string, you need to align such the int length field is aligned
	 * to {@link com.google.flatbuffers.Constants#SIZEOF_INT}, and the string data follows
	 * it directly. If all you need to do is alignment, `additional_bytes` will be 0.
	 *
	...
	public void prep(int size, int additional_bytes) {
		...
		int align_size = ~(this.bb.capacity() - this.space + additional_bytes) + 1
				& size - 1;
	...
	/**
	 * Add a `boolean` to the buffer, backwards from the current location. Doesn't align
	 * nor check for space.
	 *
	 * @param x A `boolean` to put into the buffer.
	 */
	public void putBoolean(boolean x) {
		this.bb.put(this.space -= Constants.SIZEOF_BYTE, (byte) (x ? 1 : 0));
	}
	...
	public void putByte(byte x) {
		this.bb.put(this.space -= Constants.SIZEOF_BYTE, x);
	}
	...
	/**
	 * Add a `double` to the buffer, backwards from the current location. Doesn't align
	 * nor check for space.
	 *
	 ...
	 * `FlatBuffers` compiler will create a start/end method for vector types in generated
	 * code.
	 * <p>
	 * The expected sequence of calls is:
	 * <ol>
	 * <li>Start the array using this method.</li>
	 * <li>Call {@link #addOffset(int)} `num_elems` number of times to set the offset of
	 * each element in the array.</li>
	...
	/**
	 * Finish off the creation of an array and all its elements. The array must be created
	 * with {@link #startVector(int, int, int)}.
	 *
	 ...
	 * {@link #endVector} after this method to get an offset to the beginning of vector.
	 *
	 ...
	 * array.
	 */
	public ByteBuffer createUnintializedVector(int elem_size, int num_elems,
			int alignment) {
	...
	public int createString(CharSequence s) {
		int length = s.length();
		int estimatedDstCapacity = (int) (length * this.encoder.maxBytesPerChar());
	...
	/**
	 * Structures are always stored inline, they need to be created right where they're
	 * used. You'll get this assertion failure if you created it elsewhere.
	 *
	 ...
	 * directly. The `FlatBuffers` compiler will generate helper methods that call this
	 * method internally.
	 * <p>
	 * For example, using the "Monster" code found on the "landing page". An object of
	 * type `Monster` can be created using the following code:
	 *
	 * <pre>
	 * {
	 * 	&#64;code
	 ...
	 * 	Monster.addPos(fbb, Vec3.createVec3(fbb, 1.0f, 2.0f, 3.0f, 3.0, Color.Green,
	 * 			(short) 5, (byte) 6));
	 ...
	 * Here:
	 * <ul>
	 * <li>The call to `Monster#startMonster(FlatBufferBuilder)` will call this method
	 * with the right number of fields set.</li>
	 * <li>`Monster#endMonster(FlatBufferBuilder)` will ensure {@link #endObject()} is
	 * called.</li>
	 * </ul>
	 * <p>
	 * It's not recommended to call this method directly. If it's called manually, you
	 * must ensure to audit all calls to it whenever fields are added or removed from your
	 * schema. This is automatically done by the code generated by the `FlatBuffers`
	 * compiler.
	 *
	 ...
	 * @param x A `boolean` to put into the buffer, depending on how defaults are handled.
	 * If `force_defaults` is `false`, compare `x` against the default value `d`. If `x`
	 * contains the default value, it can be skipped.
	 * @param d A `boolean` default value to compare against when `force_defaults` is
	 * `false`.
	 */
	 ...
	 * `force_defaults` is `false`, compare `x` against the default value `d`. If `x`
	 * contains the default value, it can be skipped.
	 * @param d A `byte` default value to compare against when `force_defaults` is
	 * `false`.
	 */
	 ...
	 *
	 * @param o The index into the vtable.
	 * @param x A `short` to put into the buffer, depending on how defaults are handled.
	 * If `force_defaults` is `false`, compare `x` against the default value `d`. If `x`
	 * contains the default value, it can be skipped.
	 * @param d A `short` default value to compare against when `force_defaults` is
	 * `false`.
	 */
	 ...
	 * `force_defaults` is `false`, compare `x` against the default value `d`. If `x`
	 * contains the default value, it can be skipped.
	 * @param d An `int` default value to compare against when `force_defaults` is
	 * `false`.
	 */
	 ...
	 * `force_defaults` is `false`, compare `x` against the default value `d`. If `x`
	 * contains the default value, it can be skipped.
	 * @param d A `long` default value to compare against when `force_defaults` is
	 * `false`.
	 */
	 ...
	 *
	 * @param o The index into the vtable.
	 * @param x A `float` to put into the buffer, depending on how defaults are handled.
	 * If `force_defaults` is `false`, compare `x` against the default value `d`. If `x`
	 * contains the default value, it can be skipped.
	 * @param d A `float` default value to compare against when `force_defaults` is
	 * `false`.
	 */
	 ...
	 *
	 * @param o The index into the vtable.
	 * @param x A `double` to put into the buffer, depending on how defaults are handled.
	 * If `force_defaults` is `false`, compare `x` against the default value `d`. If `x`
	 * contains the default value, it can be skipped.
	 * @param d A `double` default value to compare against when `force_defaults` is
	 * `false`.
	 */
	 ...
	 *
	 * @param o The index into the vtable.
	 * @param x An `offset` to put into the buffer, depending on how defaults are handled.
	 * If `force_defaults` is `false`, compare `x` against the default value `d`. If `x`
	 * contains the default value, it can be skipped.
	 * @param d An `offset` default value to compare against when `force_defaults` is
	 * `false`.
	 */
	...
	/**
	 * Add a struct to the table. Structs are stored inline, so nothing additional is
	 * being added.
	 *
	 ...
	 *
	 * @param voffset The index into the vtable to store the offset relative to the end of
	 * the buffer.
	 */
	...
	public int endObject() {
		...
		for (int i = this.vtable_in_use - 1; i >= 0; i--) {
		...
		addShort((short) ((this.vtable_in_use + standard_fields) * SIZEOF_SHORT));

		// Search for an existing vtable that matches the current one.
		int existing_vtable = 0;
		outer_loop: for (int i = 0; i < this.num_vtables; i++) {
		...
		if (existing_vtable != 0) {
			...
			this.vtables[this.num_vtables++] = offset();
	 ...
	 *
	 * @param root_table An offset to be added to the buffer.
	 * @param file_identifier A FlatBuffer file identifier to be added to the buffer
	 * before `root_table`.
	 */
	public void finish(int root_table, String file_identifier) {
		prep(this.minalign, SIZEOF_INT + FILE_IDENTIFIER_LENGTH);
	 ...
	 * serialized into the buffer. Forcing defaults provides a way to manually disable
	 * this optimization.
	 *
	 ...
	 * `finish()`. The actual data starts at the ByteBuffer's current position, not
	 * necessarily at `0`.
	 *
	...
	/**
	 * The FlatBuffer data doesn't start at offset 0 in the {@link ByteBuffer}, but now
	 * the {@code ByteBuffer}'s position is set to that location upon
	 * {@link #finish(int)}.
	 *
	 * @return The {@link ByteBuffer#position() position} the data starts in
	 * {@link #dataBuffer()}
	 * @deprecated This method should not be needed anymore, but is left here for the
	 * moment to document this API change. It will be removed in the future.
	 */
	@Deprecated
	...
	/**
	 * A utility function to copy and return the ByteBuffer data from `start` to `start` +
	 * `length` as a `byte[]`.
	 *
